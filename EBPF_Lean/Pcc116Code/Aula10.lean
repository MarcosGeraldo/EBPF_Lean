...
inductive Expressao : Type
| Num : ℕ -> Expressao
| Str : String -> Expressao
| Add : Expressao → Expressao → Expressao
deriving Repr

declare_syntax_cat imp_Expressao

syntax num : imp_Expressao
syntax str : imp_Expressao
syntax imp_Expressao "+" imp_Expressao : imp_Expressao


partial def elabExpressao : Syntax → MetaM Expr
/-| `(imp_Expressao| $l:imp_literal) => do
  let l ← elabliteral l
  mkAppM ``Expressao.Lit #[l]-/
| `(imp_Expressao| $n:num) => mkAppM ``Expressao.Num #[mkNatLit n.getNat]
| `(imp_Expressao| $s:str) => mkAppM ``Expressao.Str #[mkStrLit s.getString]
| `(imp_Expressao| $l:imp_Expressao + $r:imp_Expressao) => do
  let l ← elabExpressao l
  let r ← elabExpressao r
  mkAppM ``Expressao.Add #[l, r]
| _ => throwUnsupportedSyntax

elab "test_elabExpressao" e:imp_Expressao : term => elabExpressao e

#reduce test_elabExpressao 3
#reduce test_elabExpressao "3"
#reduce test_elabExpressao 3 + "3"


inductive Command : Type
| Assign : String → Expressao → Command
| Print : Expressao → Command
...
syntax ident ":=" imp_Expressao : imp_Command
syntax "print" imp_Expressao : imp_Command
syntax imp_Command ";;" imp_Command : imp_Command
syntax "nop" : imp_Command

partial def elabCommand : Syntax → MetaM Expr
| `(imp_Command| $i:ident := $r:imp_Expressao) => do
  let i : Expr := mkStrLit i.getId.toString
  let r ← elabExpressao r
  mkAppM ``Command.Assign #[i, r]
| `(imp_Command| print $i:imp_Expressao) => do
  let i ← elabExpressao i
  mkAppM ``Command.Print #[i]
| `(imp_Command| $s1:imp_Command ;; $s2:imp_Command) => do
  let s1 ← elabCommand s1
  let s2 ← elabCommand s2
  mkAppM ``Command.Seq #[s1, s2]
| `(imp_Command| nop) => do
  mkAppM ``Command.Nop #[]
| _ => throwUnsupportedSyntax

elab "{exe|" p: imp_Command "}" : term => elabCommand p
