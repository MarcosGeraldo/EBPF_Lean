...
section MEMBERSHIP
  ...
  lemma member_dist [DecidableEq A] (x : A) (xs ys : list A):
    member x (xs .++. ys) = (member x xs || member x ys) := by
    induction xs with
      | nil =>
      simp [cat]
      simp [member]
      | cons x' xs' IHxs'=>
        simp[member]
        rcases decEq x x' with _|_ <;> simp
        exact IHxs'
  ...
  lemma member_reverse [DecidableEq A](x : A)(xs : list A) :
    ...
    | cons x' xs' IHxs' =>
      ...
      --simp[reverse]
      --simp[member_dist]
      simp[reverse,member_dist,member]
      rcases decEq x x' with _ | _ <;> simp
      exact IHxs'
