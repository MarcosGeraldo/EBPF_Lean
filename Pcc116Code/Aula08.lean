...
def Finite.size (t : Finite) : ℕ :=
  match t with
  | .unit => 1
  | .either t1 t2 => size t1 + size t2
  | .pair t1 t2 => size t1 * size t2

#eval Finite.size Finite.unit --1

#eval Finite.size (Finite.either Finite.unit Finite.unit) -- 2

#eval Finite.size (Finite.pair Finite.unit Finite.unit) -- 1

#eval Finite.size (Finite.pair Finite.unit (Finite.either Finite.unit Finite.unit)) -- 2
...
def U.beq (t : U)(x y : t.asType) : Bool :=
  match t, x , y with
  | _t' , x', y' => asString x' = asString y'

def U.beq1 : ∀ (t : U), t.asType → t.asType → Bool
  | .bit, x, y => x = y
  | .char, x, y => x = y
  | .nat, x, y => x = y
  | .vec _ 0, Vec.nil, Vec.nil => true
  | .vec u (n+1), Vec.cons x xs, Vec.cons y ys => U.beq1 u x y && U.beq1 (.vec u n) xs ys

#eval U.beq .bit true false  -- Output: false
#eval U.beq .bit true true   -- Output: true
#eval U.beq .char 'a' 'a'    -- Output: true
#eval U.beq .char 'a' 'b'    -- Output: false
#eval U.beq .nat 3 3         -- Output: true
#eval U.beq .nat 3 4         -- Output: false

def exampleVec : Vec ℕ 3 := Vec.cons 1 (Vec.cons 2 (Vec.cons 3 Vec.nil))
def exampleVec2 : Vec ℕ 3 := Vec.cons 1 (Vec.cons 2 (Vec.cons 3 Vec.nil))
def exampleVec3 : Vec ℕ 3 := Vec.cons 1 (Vec.cons 2 (Vec.cons 4 Vec.nil))
def exampleVec4 : Vec ℕ 4 := Vec.cons 0 (Vec.cons 1 (Vec.cons 2 (Vec.cons 4 Vec.nil)))
def exampleVec5 : Vec ℕ 4 := Vec.cons 1 (Vec.cons 1 (Vec.cons 2 (Vec.cons 4 Vec.nil)))

#eval U.beq (.vec .nat 3) exampleVec exampleVec2  -- Output: true
#eval U.beq (.vec .nat 3) exampleVec exampleVec3  -- Output: false
#eval U.beq (.vec .nat 4) exampleVec5 exampleVec4  -- Output: false

#eval U.beq1 .bit true false  -- Output: false
#eval U.beq1 .bit true true   -- Output: true
#eval U.beq1 .char 'a' 'a'    -- Output: true
#eval U.beq1 .char 'a' 'b'    -- Output: false
#eval U.beq1 .nat 3 3         -- Output: true
#eval U.beq1 .nat 3 4         -- Output: false


#eval U.beq1 (.vec .nat 3) exampleVec exampleVec2  -- Output: true
#eval U.beq1 (.vec .nat 3) exampleVec exampleVec3  -- Output: false
#eval U.beq1 (.vec .nat 4) exampleVec5 exampleVec4  -- Output: false
